...
namespace pictureIO
{
	public partial class PictureIO : Form
	{
		...
		private void PictureInput_DragEnter(object sender, DragEventArgs e)
		{
			...
			e.Effect = DragDropEffects.None; // 不允许任何拖放
		...
		private void Generator_Click(object sender, EventArgs e)
		{
			...
			// 创建一个新的Bitmap对象用于保存调整过大小或格式的图像
			Bitmap bitmap = new Bitmap(PictureInput.Image);
			// 生成内存流
			using (MemoryStream ms = new MemoryStream())
			{
				// 将bitmap保存为JPEG格式到内存流中，同时应用JPEG质量设置
		...
		private void PictureInput_MouseClick(object sender, MouseEventArgs e)
		{
			...
			if (PictureInput.Image != null)
			{
				...
				ClickedPoint.Text = $"已选中坐标: {originalPoint.X}, {originalPoint.Y}";
			}
		}
		...
		private void TryPasteFromClipboard()
		{
			...
			if (!string.IsNullOrEmpty(clipboardText) && clipboardText.StartsWith("data:image"))
			{
				...
				{
					MessageBox.Show("无法从数据URI加载图片: " + ex.Message);
				}
			}
		}
		...
		private void ImageSherpen_Click(object sender, EventArgs e)
		{
			if (PictureInput.Image != null)
			{
				...
				Bitmap sharpenedImage = ImageSharpener.Sharpen(originalImage, 1.5f); // 调整factor值以控制锐化程度

				// 将处理后的图像设置回PictureBox
		...
		{
			if (inputsharediframebox.Text.StartsWith("<iframe"))//示例输入：<iframe src="//player.bilibili.com/player.html?isOutside=true&aid=113809956084766&bvid=BV1XacweXEC2&cid=27816494236&p=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"></iframe>
			{//预期输出：<p style = "text-align: center;"><iframe id="vplay" frameborder="0"  src="//player.bilibili.com/player.html?isOutside=true&aid=113804218271116&bvid=BV1yscHeFEMt&cid=27798341903&p=1&autoplay=0" allowFullScreen="true"></iframe></p>
